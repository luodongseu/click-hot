...
class AccessToken{
    ...
    private static function _getAccessToken(){
       ...
       // $f = fopen('access_token', 'w+');
        ...
        $sql = "SELECT * FROM `access_token` limit 1";
        $data = $mysql->getData( $sql );
        if($data){
            $sql = "UPDATE `access_token` SET `token`=".$accessTokenJson;
            $mysql->runSql($sql);
        }else{
            $sql = "INSERT INTO `access_token` ( `token`) VALUES ('".$accessTokenJson."') ";
            $mysql->runSql($sql);
        }
...
//        $accessToken = YourDatabase::get('access_token');
        ...
        $sql = "SELECT `token` FROM `access_token` limit 1";
        $data = $mysql->getData( $sql );
        $data = $data['token'];
